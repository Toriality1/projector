...
export function run() {
  program
    ...
    .option('-i, --ignore <patterns...>', 'Patterns to ignore', ['node_modules', '.git', '.DS_Store'])
    .option('-e, --extensions <extensions...>', 'File extensions to include (e.g. .py .ts)')
    ...
    .action((directory, opts) => {
      ...
      if (options.output) {
        try {
          ...
          if (options.verbose) console.log(chalk.green(`\nSaved to ${options.output}`));
        } catch (err) {
